...
// ---- štítky na záznamech (entity_list_items nad listem client_tags) ----

export async function listEntityTags(tenantId: string, entityKind: 'client' | 'person', entityId: string) {
  return db
    ...
    .select(['list_items.id as id', 'list_items.label as label'])
    .orderBy('list_items.label')
    .execute();
}
...
export async function addEntityTag(
  ...
  const item = await ensureItemByLabel(tenantId, 'client_tags', label);
  if (!item) return null;
  const link = await db
    .selectFrom('entity_list_items')
    .select('list_item_id')
...
export async function tagsForEntities(tenantId: string, entityKind: 'client' | 'person', ids: string[]) {
  if (ids.length === 0) return new Map<string, Array<{ id: string; label: string }>>();
  const rows = await db
    .selectFrom('entity_list_items')
    .innerJoin('list_items', 'list_items.id', 'entity_list_items.list_item_id')
    .where('entity_list_items.tenant_id', '=', tenantId)
  ...
  for (const r of rows) {
    ...
    map.get(r.entity_id)!.push({ id: r.id, label: r.label });
  }
  return map;
}
