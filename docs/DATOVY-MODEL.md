...
| Tabulka | Účel |
|---|---|
| `lists` | Definice číselníku: `key` (`client_statuses`, `client_tags`, `service_catalog`, `contact_types`, `roles_at_client`, `engagement_statuses`, `deal_stages`, `task_labels`…), `label`. |
...
| `work_records` | **Výkaz práce**: `client_id`, volitelně `service_id`, **volitelně `task_id`** (úkol, z něhož se vykazovalo — vazba viz `tasks`), `worker_id` (kdo pracoval), `description` (úkon = titulek), `note` (detaily, volitelně), `minutes`, `performed_at`, **`status` pending / approved / returned / rejected**, `approved_by_id`, `approved_at`, **`rejection_reason`** (poznámka schvalovatele: instrukce u *returned*, důvod u *rejected*; uvidí ji pracovník). Stavy: *pending* čeká · *approved* schváleno · **returned** vráceno k přepracování (rework — pracovník opraví a znovu pošle) · **rejected** zamítnuto (terminální, záznam zůstává — „nic se nemaže"). **Do času a peněz vstupují jen pending + approved.** Každý záznam má dohledatelné ID; při založení se automaticky vytvoří úkol pro odpovědnou osobu (schválení). |
...
| `tasks` | Úkol: titulek, **štítky** (M:N přes `entity_list_items` entity_kind=`task`, Seznam `task_labels` — víc na úkol; sloupec `category_item_id` je legacy/nevyužitý), volitelně klient/zakázka/deal, termín `due_at`, hotovo, přiřazený. **Kanban (v2):** `status_id`→`task_statuses` (sloupec; null = odvodí se z `done`), `prev_status_id` (návrat po odškrtnutí), `archived` 0/1, `board_month` (`YYYY-MM`; null = trvalý/osobní board), `sort_order`. „Vyřízeno" (`done`) ⇄ stav s `is_done` se drží v synchronu. |
...
| `notifications` | **Notifikace (modul D)** — *adresná* schránka „co se týká tebe" (na rozdíl od *neadresného* logu `events`): `recipient_id` (komu), `actor_id` (kdo akci vyvolal), `type` (`work_record_approved`/`_returned`/`_rejected`/`_pending`/`task_assigned`), `title` (česky = i tr-klíč pro EN) + `body` (detail = data), `entity_kind`+`entity_id`, `link` (kam vede klik), **`read_at`** (čas přečtení; null = nepřečteno — **měkký stav, „nic se nemaže"**). Vznik se vysílá realtime → odznak u zvonku se sám rozsvítí. Nikdy neupozorňuje sám sebe (`actor==recipient` → přeskočí). Producenti: rozhodnutí o výkazu (autorovi), nový výkaz ke schválení (vlastník klienta + admini), přidělení úkolu. V panelu se `work_record_pending`/`_approved` **slučují** do řádku s počtem. Doména `src/domain/notifications.ts`, UI `src/web/notifikace.tsx`, spec `docs/specs/notifikace-v1.md`. |
