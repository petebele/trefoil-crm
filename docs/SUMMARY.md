...
> samostatně** (knowledge base, mimo feed Aktivity). Navíc: **„Vytvořit úkol" z poznámky už není
> automatické** — otevře **modál nového úkolu** předvyplněný (název + klient + skrytá **vazba na
> poznámku** `source_kind='note'`), kde se dopíše řešitel/termín/detaily; auto-create routa zrušena. **Další na řadě:** ověřit v prohlížeči; pak globální
...
> „Komunikace"**, ne ve v1. Navíc na **Nástěnce firmy i osoby** blok „Poslední dění" → **„Poslední
> aktivity"** (stejný vzhled feedu = `ActivityList` + odkaz „Zobrazit všechny → Aktivity"). **Další na
> řadě:** ověřit „Aktivity" v prohlížeči; pak modul Komunikace / detail-dashboard služby / příležitosti
> dle [VIZE feedu](VIZE-feed-a-prilezitosti.md); pozn.: úvodní (globální) Nástěnka — sjednotit „Poslední
> dění" do stejného vzhledu + rozšířit na Inbox „Vyžaduje moji pozornost" (zatím nehotovo).
>
