...
Tento soubor je „rozcestník a celkový obraz". Detailní dokumentace je v docs/:
  - docs/UI-ZASADY.md     — designový manuál (replika Capsule CRM), barvy/skiny
  - docs/KOMPONENTY.md    — katalog znovupoužitelných UI komponent (kódy + pravidla)
  - docs/DATOVY-MODEL.md  — datový model (Person/Client/Seznamy/RBAC, billing)
  - docs/SLOVNIK.md       — jednotná terminologie (pojmy = stejná slova v UI i kódu)
  - docs/specs/           — specifikace jednotlivých modulů (vznikají PŘED stavbou)
  - mockupy/*.html        — klikací vizuální reference obrazovek (otevři v prohlížeči)
  - solution-summary.txt  — historie projektu, rozhodnutí a plán dalších fází
...
Konfigurace (vše má default, .env není potřeba) — src/config.ts:
  ...
  sessionTtlDays = 30 (platnost přihlášení)
...
-------------------------------------------------------------------------------
  ...
  - Tabulky (schema.ts): tenants, tenant_modules, persons, sessions, clients,
    ...
    services, work_records. (Detailní význam: docs/DATOVY-MODEL.md.)
...
-------------------------------------------------------------------------------
  ...
  Detaily: docs/UI-ZASADY.md §2 a docs/KOMPONENTY.md §23.
...
-------------------------------------------------------------------------------
  ...
  Detaily a důvody: solution-summary.txt + docs/DATOVY-MODEL.md.
